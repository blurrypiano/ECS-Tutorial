...
namespace std {
template <>
struct hash<ecs::EntQuery__internal> {
  size_t operator()(ecs::EntQuery__internal const &query) const { return query.hashValue; }
};
}  // namespace std
...
class EntQuery {
 ...
 private:
  ...
  EntQuery__internal build() {
    // calculate hashcode
    size_t hashValue = 0;
    for (const auto componentType : allOfTypes) ecs::hashCombine(hashValue, componentType);
    for (const auto componentType : anyOfTypes) ecs::hashCombine(hashValue, componentType);
    for (const auto componentType : noneOfTypes) ecs::hashCombine(hashValue, componentType);
